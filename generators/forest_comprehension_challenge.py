...
for location in locations:
    print([(key, locations[key]) for key in locations.keys() if location in exits[key].values()])
    print("*" * 40)

# the same output as before with foor loops instead of comprehensions
for location in locations:
    res = []
    for key in locations.keys():
        if location in exits[key].values():
            res.append((key, locations[key]))
    print(res)
    print("*" * 40)
